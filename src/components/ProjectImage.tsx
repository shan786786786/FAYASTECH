...
export default function ProjectImage({ category, title, className = "", customStyle }: ProjectImageProps) {
  const styleToUse = customStyle || category
  const Icon = categoryIcons[category as keyof typeof categoryIcons] || categoryIcons.default
  const gradient = categoryGradients[styleToUse as keyof typeof categoryGradients] || categoryGradients.default
