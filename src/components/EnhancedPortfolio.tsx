...
const ProjectCard = ({ project, index }: { project: typeof projects[0], index: number }) => {
  ...
  return (
    ...
    >
      ...
      >
        <motion.div
          className="relative bg-gradient-to-br from-gray-800/80 to-gray-900/80 backdrop-blur-xl rounded-2xl overflow-hidden border border-gray-700/50 hover:border-cyan-500/50 transition-all duration-300"
        ...
        >
          ...
          {project.featured && (
            <motion.div
              className="absolute top-4 right-4 z-10 bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-3 py-1 rounded-full text-xs font-semibold"
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
          ...
          <div className="p-6">
            <motion.h3 
              className="text-xl font-bold text-white mb-2 group-hover:text-cyan-500 transition-colors"
              layout
            >
              {project.title}
            </motion.h3>
            ...
            >
              {project.tags.map((tag, tagIndex) => (
                <motion.span
                  key={tag}
                  className="px-2 py-1 bg-gray-700/50 text-cyan-500 text-xs rounded-lg border border-gray-600/50"
            ...
            >
              <Button
                variant="neon"
                size="sm"
                className="flex-1 group/btn shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/40 transition-shadow"
                onClick={() => {
                  if (project.liveUrl) {
              ...
              <Button
                variant="outline"
                size="sm"
                className="border-cyan-500/50 text-cyan-400 hover:text-white hover:bg-cyan-500/10 hover:border-cyan-500 transition-all shadow-lg"
                onClick={() => {
                  if (project.liveUrl) {
                    window.open(project.liveUrl, '_blank')
                  } else {
